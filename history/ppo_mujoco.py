...
def train(env, policy, optimizer, discount_factor, ppo_steps, ppo_clip):
    ...
    returns = returns.to(args.device).float()
    values = values.to(args.device).float()
...
def update_policy(policy, states, actions, log_prob_actions, advantages, returns, optimizer, ppo_steps, ppo_clip):
    states = states.to(args.device).float()
    actions = actions.to(args.device).float()
    log_prob_actions = log_prob_actions.to(args.device).float()
    advantages = advantages.to(args.device).float()
    returns = returns.to(args.device).float()
...
def evaluate(env, policy):
    ...
    while not done:
        ...
        state = torch.FloatTensor(state).unsqueeze(0)
        state = state.to(args.device).float()
        with torch.no_grad():
...
train_env = load_environment('halfcheetah-medium-expert-v2')
test_env = load_environment('halfcheetah-medium-expert-v2')
...
actor = MLP(INPUT_DIM, HIDDEN_DIM, OUTPUT_DIM).to(args.device)
critic = MLP(INPUT_DIM, HIDDEN_DIM, 1).to(args.device)
