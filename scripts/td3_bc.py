...
path ="/home/fernandi/projects/diffuser/trajectories/safe_grid_2000__rate_0.93.pickle"
#read pickle
...
name_run = "TD3_BC_normalized_512"
@dataclass
class TrainConfig:
    # Experiment
    device: str = "cuda:4"
    env: str = "SafeGrid-v0"  # OpenAI gym environment name
    seed: int = 1234556  # Sets Gym, PyTorch and Numpy seeds
    eval_freq: int = int(5e3)  # How often (time steps) we evaluate
    n_episodes: int = 100  # How many episodes run during evaluation
    max_timesteps: int = int(1e6)  # Max time steps to run environment
    checkpoints_path: Optional[str] = f"/home/fernandi/projects/diffuser/logs/{name_run}"  # Save path
...
def wrap_env(
    ...
    env = wrappers.TransformObservation(env, normalize_state)
    if reward_scale != 1.0:
        env = wrappers.TransformReward(env, scale_reward)
    return env
...
def eval_actor(
    ...
    actor.train()
    return np.array(episode_rewards), np.array(episode_costs), success_rate/n_episodes
...
def train(config: TrainConfig):
    ...
    for t in range(int(config.max_timesteps)):
        ...
        if (t + 1) % config.eval_freq == 0:
            ...
            eval_scores, eval_costs, suceess_rate = eval_actor(
                env,
                actor,
                device=config.device,
            ...
            print(
                ...
                f" , Cost: {eval_cost:.3f}, Success Rate: {suceess_rate:.3f}"
            )
            print("---------------------------------------")
            ...
            wandb.log(
                {"return": eval_score, "cost": eval_cost, "success_rate": suceess_rate},
